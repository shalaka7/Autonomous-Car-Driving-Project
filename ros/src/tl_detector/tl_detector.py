...
#         rospy.loginfo("===============Got image from car mounted camera..===============")
...
#             rospy.loginfo("Detector state: {}".format(self.to_string(state)))
        else:
...
#             rospy.loginfo("Detector state: {}".format(self.to_string(state)))
    ...
    def get_light_state(self, light):
        ...
        """
        # Get classification. Not needed for simulator. Classifier will be only used with real car.
...
#         rospy.loginfo('Light state: %s', light.state)
    ...
    def process_traffic_lights(self):
        ...
        if (self.pose):
            # rospy.loginfo("Pose: x={}, y={}".format(self.pose.pose.position.x, self.pose.pose.position.y))
        ...
        for i, light in enumerate(self.lights):
            ...
            # Update members with valid lights to track if any.
            if 0 <= d < diff:
                diff = d
                closest_light = light
